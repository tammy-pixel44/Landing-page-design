  ...
  return (
    <div className="decision-console" ref={shellRef}>
      <div className="decision-console__topline">
        <span>LIVE PRODUCT PROTOTYPE</span>
        <span>DEMO FIXTURES · NOT FINANCIAL ADVICE</span>
      </div>
      ...
      <div className="decision-console__grid">
            ...
            <div className="scenario-list" role="tablist" aria-label="Demo purchase scenarios">
              {demoScenarios.map((item, index) => (
                <button
                  key={item.id}
                  type="button"
                  className={item.id === scenario.id ? "scenario-choice is-active" : "scenario-choice"}
        ...
        <div className="decision-result">
          <div className="decision-result__intro">
            <span className="micro-label">03 / KACHING ANSWER</span>
            <span className="decision-status"><i /> wallet evaluated</span>
          </div>

          <div className="decision-winner">
            <div>
              <span className="winner-overline">USE THIS CARD</span>
              <h3>{ranked[0].name}</h3>
              <p>{ranked[0].reason}</p>
            </div>
            <div className="winner-number" ref={winnerRef}>
              <small>EXPECTED DEMO VALUE</small>
              <strong>{inr.format(ranked[0].liveReward)}</strong>
              <span>{winnerDelta > 0 ? `+${inr.format(winnerDelta)} vs #2` : "best available fixture"}</span>
            </div>
          </div>
          ...
          <p className="decision-disclosure">
            The interaction demonstrates KaChing&apos;s decision pattern. Card names and figures here are invented UX fixtures; production recommendations must come from the real wallet + reward engine.
          </p>
        </div>
      </div>
    </div>
