...
export default function DecisionLab() {
  ...
  useEffect(() => {
    ...
    return () => {
      rowsAnimation.cancel();
      rewardAnimation?.cancel();
    };
  }, [scenarioId, amount]);
  ...
  return (
    <div className="decision-console" ref={shellRef}>
      <div className="decision-console__topline">
        <span>CARDEIFY DECISION PREVIEW</span>
        <span>SAMPLE DATA · NOT FINANCIAL ADVICE</span>
      </div>
      <div className="decision-console__grid">
        <div className="decision-inputs">
          <div className="decision-input-group">
            ...
            <div className="scenario-list" role="tablist" aria-label="Sample purchase scenarios">
        ...
        <div className="decision-result">
          ...
          <div className="decision-winner">
            <div><span className="winner-overline">USE THIS SAMPLE CARD</span><h3>{ranked[0].name}</h3><p>{ranked[0].reason}</p></div>
            <div className="winner-number" ref={winnerRef}><small>EXPECTED SAMPLE VALUE</small><strong>{inr.format(ranked[0].liveReward)}</strong><span>{winnerDelta > 0 ? `+${inr.format(winnerDelta)} vs #2` : "best available sample"}</span></div>
          </div>
          <div className="ranking-table" aria-live="polite">
            {ranked.map((card, index) => {
              ...
              return (
                <div className="ranking-row" key={card.name}>
                  <span className="ranking-index">0{index + 1}</span>
                  <span className={`ranking-card-swatch ranking-card-swatch--${card.accent}`} aria-hidden="true"><i /></span>
                  <div className="ranking-identity"><strong>{card.name}</strong><span>{scenario.category}</span></div>
                  <div className="ranking-meter" aria-hidden="true"><i style={{ width: `${width}%` }} /></div>
                  <strong className="ranking-value">{inr.format(card.liveReward)}</strong>
                </div>
              );
            })}
          </div>
          <p className="decision-disclosure">This interaction demonstrates the decision pattern using invented card names and sample reward figures. A live Cardeify recommendation must be calculated from the user&apos;s wallet and current reward rules.</p>
        </div>
      </div>
    </div>
  );
