...
export default function PosDemo() {
  ...
  useEffect(() => {
    ...
    let resultAnimation: ReturnType<typeof animate> | null = null;
    if (step === screens.length - 1 && resultRef.current) {
      resultAnimation = animate(resultRef.current, {
        opacity: { from: 0 },
        y: { from: 18 },
        scale: { from: 0.97 },
        duration: 560,
    ...
    <div className="checkout-demo">
      ...
      <div className="terminal-wrap">
        <div className="terminal-pro">
          ...
          <div className="terminal-pro__meta">
            <span><i /> KACHING ADVISORY</span>
        ...
        <div className="checkout-result" ref={resultRef} data-visible={step === screens.length - 1}>
          <span>BEST CARD FOR THIS DEMO PURCHASE</span>
          <div><strong>Everyday Plus</strong><b>₹62</b></div>
          <small>Dining multiplier wins before the demo cap.</small>
        </div>
      </div>

      <div className="checkout-controls">
        {step === 0 ? (
          <button type="button" className="primary-action primary-action--cream" onClick={run}>
            <span>Run the payment decision</span><i>↗</i>
          </button>
        ) : (
          <button type="button" className="secondary-action" onClick={reset} disabled={running}>
            {running ? "Evaluating the wallet…" : "Run it again"}
          </button>
        )}
      </div>
    </div>
